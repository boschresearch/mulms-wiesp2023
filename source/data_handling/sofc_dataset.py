...
# Thank you to https://github.com/ZacharySBrown for creating this HuggingFace dataset script!
# File adapted from https://huggingface.co/datasets/sofc_materials_articles/blob/main/sofc_materials_articles.py

"""
This module contains the dataset class for the SOFC dataset.
"""
...
class SOFCMaterialsArticles(datasets.GeneratorBasedBuilder):
                ...
                "spans": datasets.features.Sequence(
                    {
                        ...
                        "entity_label": datasets.features.ClassLabel(
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.data_dir is None:
            ...
            data_dir = os.path.join(
                data_dir, "sofc-exp_textmining_resources-master/sofc-exp-corpus"
            )
        else:
        ...
        text_files_available = [
            os.path.split(i.rstrip(".txt"))[-1]
            for i in glob.glob(os.path.join(text_base_path, "*.txt"))
        ]
    ...
    def _generate_examples(self, names, data_dir, split):
        ...
        for id_, name in enumerate(sorted(names)):
            ...
            entity_labels = (
                ets_meta.groupby("sentence_id")
                .apply(lambda x: x["entity_label"].tolist())
                .to_list()
            )
            slot_labels = (
                ets_meta.groupby("sentence_id").apply(lambda x: x["slot_label"].tolist()).to_list()
            )
            ...
            sentence_offsets = (
                sentence_meta[["begin_char_offset", "end_char_offset"]]
            ...
            sentences = [
                text[ost["begin_char_offset"] : ost["end_char_offset"]] for ost in sentence_offsets
            ]
            ...
            experiment_starts = [
                i for i, line in enumerate(frames_meta) if line.startswith("EXPERIMENT")
            ]
            ...
            # Manually adding end of last experiment to the list
            for start, end in zip(experiment_starts, experiment_starts[1:] + [link_start]):
                ...
                for e in current_experiment[1:]:
                    ...
                    to_add = {
                        "frame_participant_label": frame_participant_label,
                        "slot_id": int(slot_id),
                    }
                    slots.append(to_add)
                exp["slots"] = slots
