...
class PairwiseBilinear(nn.Module):
    ...
    def __init__(self, input1_size, input2_size, output_size, bias=True):
        ...
        self.weight = nn.Parameter(
            torch.zeros(input1_size, input2_size, output_size), requires_grad=True
        )
    ...
    def forward(self, input1, input2):
        ...
        output = (
            output.view(input1_size[0], input1_size[1], self.output_size, input2_size[1])
            .transpose(2, 3)
            .contiguous()
        )
...
class DeepBilinearScorer(nn.Module):
    def __init__(
        self, input1_size, input2_size, hidden_size, output_size, hidden_func=F.relu, dropout=0.0
    ):
        super().__init__()
